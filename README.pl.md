...
226. **Zdarzenia Alarmo były ucinane** — `HA_ALARM_EVENT_TYPE_LEN` wynosiło 32, więc zapis `"ready to arm modes updated"` (25 znaków + kończący `\0` + zapas) potrafił zostać przycięty i porównanie `strcmp` z oczekiwanym typem zdarzenia nie trafiało. Skutek: kafelek alarmu **nie odświeżał** maski gotowości do uzbrojenia, listy blokujących czujek ani powodu odrzucenia. Bufor zwiększono do 48 znaków i zabezpieczono `_Static_assert`em, żeby skrócenie stałej nie przeszło niezauważone.
227. **Błąd kompilacji `format-truncation` w kafelku alarmu** — linia statusu „Arming failed: …” była budowana w jednym `snprintf`, a GCC w buildach debug (`-Og`) potrafi ostrzeżenie o możliwym ucięciu zamienić w błąd (`-Werror`). Tekst składa się teraz z `snprintf(..., "%s", title)` i `alarm_text_appendf(..., ": %s", detail)`, więc analiza formatu nie widzi już sumy argumentów. Build debug i release są czyste.
228. **Wyrównanie konfiguracji builda** — `sdkconfig.defaults.panels3` zawiera teraz opcje, które wcześniej istniały tylko w lokalnym `sdkconfig` autora, więc klon repozytorium buduje firmware **funkcjonalnie identyczny** z panelem na ścianie, a nie tylko podobny: długie nazwy plików FATFS (`HEAP`, 255, UTF-8, strona kodowa **852**) — bez tego wygaszacz nie odczytałby obrazków z microSD, okno RX block-ack Wi-Fi `6` i kolejka TCP OOO `4` — stabilność przy jednoczesnym HA WebSocket, MQTT i edytorze WWW, zaokrąglanie mieszania kolorów RGB565 (`128`), wyłączony cache stylów obiektów LVGL, wyłączony motyw mono oraz włączone asercje LVGL (błąd kończy się backtrace'em, a nie śmieciami w buforze obrazu). Uwaga: `CONFIG_FATFS_CODEPAGE` jest **wyborem** w Kconfig, więc działa tylko `CONFIG_FATFS_CODEPAGE_852=y`; wpis `CONFIG_FATFS_CODEPAGE=852` jest po cichu ignorowany.
229. **Wgrane pliki firmware przebudowane z poprawkami** — aplikacja `betta-ha-panel-s3.bin` **3 251 328 B**, scalony obraz `guition-4-esp32s3.factory.bin` **3 382 400 B**, odświeżony też `bootloader.bin`; w `firmware/README.md` zapisane sumy `SHA256`. Build: **0 błędów**, ostrzeżenia wyłącznie z przestarzałych API LVGL (400), a hałaśliwe ostrzeżenie o przestarzałym `LV_ASSERT_HANDLER_INCLUDE` (942 wystąpień) jest wyciszone. Kopia robocza nie zawiera żadnych danych prywatnych (SSID, adresy IP, dane MQTT) — sprawdzone także po dekompresji trzech osadzonych strumieni gzip WebUI.
