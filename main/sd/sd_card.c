 ...
 *
 * microSD card on the ESP32-4848S040 (panels3) board.
 *
 * The TF socket is wired to an SPI bus (IO48 SCK, IO47 MOSI, IO41 MISO,
 * IO42 CS) as documented by the manufacturer demo, and IO47/IO48 are shared
 * with the bit-banged ST7701 init sequence.  `display_init()` asks the RGB
 * panel driver to delete its IO descriptors after init
 ...
 * running is only noticed by trying again.  Failures are retried quickly at
 * first (the user is probably standing at the panel) and then rarely. */
static void auto_mount_task(void *arg)
{
    (void)arg;
    unsigned attempts = 0;
    for (;;) {
        vTaskDelay(pdMS_TO_TICKS(attempts < 5U ? 3000 : 15000));
