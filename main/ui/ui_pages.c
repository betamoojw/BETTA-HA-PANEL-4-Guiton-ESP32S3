...
/* Chain the elements from the left (date, gear) and from the right (status
 * chips), then fit the clock into the gap that is left over. */
static void ui_topbar_apply_layout(void)
{
    ...
    if (show_gear) {
        lv_obj_align(s_gear_icon, LV_ALIGN_LEFT_MID, left, 0);
        left += lv_obj_get_width(s_gear_icon) + TOPBAR_GAP;
    }
...
static void ui_pages_create_topbar(lv_obj_t *screen)
{
    ...
    /* Small settings gear between the date and the clock. */
    s_gear_icon = lv_label_create(s_topbar);
    lv_obj_remove_style_all(s_gear_icon);
    lv_obj_set_size(s_gear_icon, 28, 28);
    lv_obj_align(s_gear_icon, LV_ALIGN_LEFT_MID, 142, 0);
